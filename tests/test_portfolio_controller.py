...
def test_safe_mode_restricts_allocations() -> None:
    limits = PortfolioLimits(max_portfolio_r_pct=1.5, max_abs_correlation=0.6, max_high_corr_positions=1, safe_mode_r_multiplier=0.5)
    controller = PortfolioController(limits=limits)
    ...
    # в safe-mode разрешаем только половину портфельного риска
